...
public:
	...
	*/
	...
	std::string GetAsString() const {
		...
		if (!IsNull()) {
			if (IsBool()) {
				...
				ss << std::string(GetString(), GetStringLength());
		}
		return ss.str();
	}
	...
	std::string GetFieldAsString(const std::string & fieldName, const std::string & defaultValue) const {
		if (HasMember(fieldName.c_str())) {
			const GenericValue & jsonValue = (*this)[fieldName.c_str()];
